...
from Components.Renderer.PremiumPosterXDownloadThread import PremiumPosterXDownloadThread
...
def convtext(text=''):
    try:
        if text != '' or text is not None or text != 'None':
            ...
            text = re.sub(r'\ \(\d+\/\d+\)$', '', text)  # remove episode-number " (xx/xx)" at the end
            text = re.sub(r'\ \(\d+\)$', '', text)  # remove episode-number " (xxx)" at the end
...
class PosterDB(PremiumPosterXDownloadThread):
    def __init__(self):
        PremiumPosterXDownloadThread.__init__(self)
        self.logdbg = None
...
class PosterAutoDB(PremiumPosterXDownloadThread):
    def __init__(self):
        PremiumPosterXDownloadThread.__init__(self)
        self.logdbg = None
...
class PremiumPosterX(Renderer):
    def __init__(self):
        # NOTE: this used to bail out here if intCheck() (a blocking 5s call to
        # google.com) failed, leaving self.instance/self.canal/self.pstcanal unset.
        # changed()/showPoster()/waitPoster() always assume those attributes exist,
        # so a slow/blocked network check used to crash every poster widget with
        # AttributeError. The connectivity check isn't used anywhere else, so it's
        # safe to always finish initialization here.
    ...
    def showPoster(self):
        if self.instance:
            self.instance.hide()
        if self.canal[5]:
            if not hasattr(self, 'pstcanal') or not os.path.exists(self.pstcanal):
                pstcanal = convtext(self.canal[5])
                pstrNm = self.path + '/' + pstcanal + ".jpg"
                self.pstcanal = str(pstrNm)
            if os.path.exists(self.pstcanal):
        ...
        if self.canal[5]:
            ...
            while loop >= 0:
                ...
                try:
                    ...
                    # renderer/widget was torn down (e.g. fast channel zapping) while this
                    # background thread was still waiting for the poster download
                    break
                time.sleep(0.5)
                loop = loop - 1
            if found:
    ...
    def logPoster(self, logmsg):
        try:
            w = open("/tmp/PremiumPosterX.log", "a+")
            w.write("%s\n" % logmsg)
            w.close()
        except Exception as e:
