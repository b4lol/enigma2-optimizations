...
class PremiumPosterXDownloadThread(threading.Thread):
    def __init__(self):
        # NOTE: used to skip threading.Thread.__init__() here when intCheck()
        # (a blocking 5s call to google.com) failed. Subclasses are started via
        # .start() right after construction, which raises RuntimeError if the
        # base Thread.__init__ never ran, so always finish initialization.
